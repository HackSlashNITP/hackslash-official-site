...
const Admin = async () => {
    ...
    await connectToDb();
    const blogs = await Post.find().populate({ path: 'author', select: 'username' });
    const events = await Event.find().populate({ path: 'author', select: 'username' });
            ...
            <div className='flex flex-col gap-8 lg:flex-row'>
                ...
                <div className='flex-1 flex flex-col gap-2 lg:max-w-[33.33%]'>
                    ...
                    <div className='h-[500px] bg-gradient-to-r from-teal-500 via-cyan-600 to-blue-700 rounded-2xl p-4 flex flex-col gap-4 overflow-y-scroll'>
                        {
                            events.map((event) => (
                                <div key={event._id} className='flex gap-4 text-2xl text-black bg-[rgba(255,255,255,0.3)] p-2 rounded-2xl'>
                                    <div className='w-32 h-28 relative'>
                                        <Image src={event.images[0]} alt='Blog_thumbnail' className='rounded-2xl object-cover' fill></Image>
                                    </div>
                                    ...
                                    <div className='flex-1 min-w-0'>
                                        ...
                                        <div className='flex gap-2 text-xl'>
                                            <Link href={'/events/' + event._id} className='cursor-pointer rounded bg-green-700 text-white px-1'>View</Link>
                                            <form action={handleEventDelete.bind(null, event._id.toString())}>
                                                <button className='rounded bg-red-500 text-white px-1 cursor-pointer'>Delete</button>
                                            </form>
                                        </div>
                                    </div>
                                </div>
                ...
                <div className='flex-1 flex flex-col gap-2 lg:max-w-[33.33%]'>
                    ...
                    <div className='h-[500px] bg-gradient-to-r from-[#009245] to-[#FCEE21] rounded-2xl p-4 flex flex-col gap-4 overflow-y-scroll'>
                        {
                            blogs.map((blog) => (
                                <div key={blog._id} className='flex gap-4 text-2xl text-black bg-[rgba(255,255,255,0.3)] p-2 rounded-2xl'>
                                    <div className='w-32 h-28 relative'>
                                        <Image src={blog.images[0]} alt='Blog_thumbnail' className='rounded-2xl object-cover' fill></Image>
                                    </div>
                                    ...
                                    <div className='flex-1 min-w-0'>
                                        ...
                                        <div className='flex gap-2 text-xl'>
                                            <Link href={'/blogs/' + blog._id} className='rounded cursor-pointer bg-green-700 text-white px-1'>View</Link>
                        ...
                        <div className='p-4 flex flex-col gap-4'>
                            {
                                users.map((user) => (
                                    <div key={user._id} className='flex items-center gap-2 bg-[rgba(255,255,255,0.3)] rounded-3xl p-1'>
                                        <Image src={'/user.png'} width={48} height={48} className='object-cover overflow-hidden' alt='User'></Image>
