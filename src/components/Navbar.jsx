    ...
    <>
      <div className="py-2 md:py-0 w-full h-auto z-50 bg-clip-padding backdrop-filter backdrop-blur-sm bg-opacity-0 sticky top-0 grid grid-cols-3 items-center ">
        <div className="ml-[20px] sm:ml-[30px] md:ml-[50px] xl:ml-[90px] col-span-1">
          <Link href={"/"}>
            <Image
              className="transform scale-90 md:scale-100 md:pt-2 my-2 sm:my-3 xl:my-4"
              src="/staticAssets/images/hackslash-logo.png"
              alt="HackSlash Logo"
              width={150}
              height={30}
            />
          ...
          {List.map((link, index) => (
            <div key={index} className="relative hidden md:flex">
              <Link
                key={index}
                href={link.path}
                className="text-base text-white lg:text-xl hover:text-slate-300 transition-all"
          ...
          <button className="md:hidden" onClick={toggleDropdown}>
            <div className="relative h-6 w-6">
              <svg
                ...
                strokeWidth={1.5}
                stroke="currentColor"
                className={`absolute text-white transition-transform duration-500 ease-in-out transform ${
                  isDropdownOpen
